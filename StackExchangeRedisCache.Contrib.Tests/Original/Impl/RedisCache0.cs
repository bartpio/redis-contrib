...
using StackExchangeRedisCache.Contrib.Tests.Original.Impl;
...
/// </summary>
/// <remarks>
/// Modified and non-functional variant of <see cref="RedisCache"/> that uses specific <see cref="WrappedDatabase"/> implementation instead of <see cref="IDatabase"/>.
/// </remarks>
public class RedisCache0 : IDistributedCache, IDisposable
{
    ...
    private volatile WrappedDatabase? _cache;
    ...
    public RedisCache0(IOptions<RedisCacheOptions> optionsAccessor)
        : this(optionsAccessor, Microsoft.Extensions.Logging.Abstractions.NullLoggerFactory.Instance.CreateLogger<RedisCache>())
    {
    }
    ...
    internal RedisCache0(IOptions<RedisCacheOptions> optionsAccessor, ILogger logger)
    {
    ...
    private WrappedDatabase Connect()
    {
        CheckDisposed();
        var cache = _cache;
        ...
        try
        {
            ...
            if (cache is null)
            {
                ...
                cache = _cache = connection.GetDatabase() as WrappedDatabase;
            }
            Debug.Assert(_cache is not null);
            return cache;
        }
    ...
    private ValueTask<WrappedDatabase> ConnectAsync(CancellationToken token = default)
    {
        CheckDisposed();
        token.ThrowIfCancellationRequested();
    ...
    private async ValueTask<WrappedDatabase> ConnectSlowAsync(CancellationToken token)
    {
    ...
    private void Refresh(WrappedDatabase cache, string key, DateTimeOffset? absExpr, TimeSpan? sldExpr)
    {
        ArgumentNullThrowHelper.ThrowIfNull(key);
    ...
    private async Task RefreshAsync(WrappedDatabase cache, string key, DateTimeOffset? absExpr, TimeSpan? sldExpr, CancellationToken token = default)
    {
        ArgumentNullThrowHelper.ThrowIfNull(key);
...
#pragma warning restore CA2208 // Instantiate argument exceptions correctly
    ...
    private void OnRedisError(Exception exception, WrappedDatabase cache)
    {
    ...
    static void ReleaseConnection(WrappedDatabase? cache)
    {
