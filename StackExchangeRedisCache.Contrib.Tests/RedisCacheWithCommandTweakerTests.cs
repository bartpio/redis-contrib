...
public sealed class RedisCacheWithCommandTweakerTests : IClassFixture<RedisFixture>, IDisposable
{
    public const string InstanceName = "RedisCacheWithCommandTweakerTests";

    /// <summary>When tests map writes to <see cref="CommandFlags.FireAndForget"/>, allow commands to complete before reads.</summary>
    private static readonly TimeSpan AfterFireAndForgetWriteDelay = TimeSpan.FromMilliseconds(100);

    private readonly ICommandFlagsTweaker _tweaker;
    private readonly ServiceProvider _sp;
    private readonly IDistributedCache _cache;

    public RedisCacheWithCommandTweakerTests(RedisFixture redis)
    {
        ...
        services.AddStackExchangeRedisCache(
            rco =>
            {
                rco.ConfigurationOptions = ConfigurationOptions.Parse(redis.ConnectionString);
    ...
    public async Task SetAndGetReturnsObjectViaInternalBatchImpl()
    {
        ...
        await Task.Delay(AfterFireAndForgetWriteDelay);
    ...
    public void SetAndGetReturnsObjectViaInternalBatchImplSync()
    {
        ...
        _tweaker.DidNotReceiveWithAnyArgs().TweakGetType(default, default);
        _tweaker.Received(2).TweakSetType(CommandFlags.None, new RedisKey("myKey").Prepend(InstanceName));
        Thread.Sleep(AfterFireAndForgetWriteDelay);
        _tweaker.ClearReceivedCalls();
